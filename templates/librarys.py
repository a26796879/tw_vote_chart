...
def all_towntup_vote_data():
    dirs = os.listdir('vote_datasets/111年直轄市議員/')
    print(dirs)
    for file_name in dirs:
        if '.xls' in file_name:
            ...
            if city != file_name:
                try:
                    os.mkdir(f'vote_datasets/111年直轄市議員/{city}')
                except:
                    pass

        xl = pd.ExcelFile(f'vote_datasets/111年直轄市議員/{file_name}')
        for sheet_name in xl.sheet_names:  # 歷遍每個sheet
            ...
            change_name = {colNameList[0]: 'village_name',
                           colNameList[1]: '村里別', colNameList[2]: '投開票所別'}
            for i in range(3, candidate_col):
                if len(str(df.iloc[1, i])) > 3:     #避免有空號候選人
            ...
            with open(f'vote_datasets/111年直轄市議員/{city}/{sheet_name}.js', 'w') as f:
